...
export function CheckTestWorkContent({ data }: CheckTestWorkContentProps) {
  ...
  const handleAllowRetake = () => {
    // toastPromise({
    //   handler: retakeTest({ test_id: data.test_id!, student_id: id }),
  ...
  const handleSetMark = () => {
    // toastPromise({
    //   handler: testMark({ test_id: data.test_id!, user_id, mark }),
  ...
  return (
    <div className={'lesson-section__block'}>
      ...
      {/*      {data?.answers.result.map((answer, index) => (*/}
      ...
      {/*            <span className="dropdawn__button-value">{data?.mark}</span>*/}
      {/*          </div>*/}
      {/*        )}*/}
      {/*      </div>*/}
      {/*      {isEditing ? (*/}
